...
export class MiniMap {
  ...
  constructor(parent: Container, worldWidth: number, worldHeight: number) {
    ...
    this.bg.rect(0, 0, MINI_WIDTH, MINI_HEIGHT);
    this.bg.fill({ color: 0x000000, alpha: 0.5 });
    this.bg.rect(0, 0, MINI_WIDTH, MINI_HEIGHT);
    this.bg.stroke({ color: COLORS.textDim, width: 1 });
  ...
  update(
    ...
    gfx.moveTo(0, miniFinishY);
    gfx.lineTo(MINI_WIDTH, miniFinishY);
    gfx.stroke({ color: COLORS.gold, width: 1, alpha: 0.8 });
    ...
    gfx.stroke({ color: COLORS.text, width: 1, alpha: 0.7 });
    ...
    for (const marble of marbles) {
      ...
      gfx.circle(mx, my, radius);
      gfx.fill({ color: marble.color, alpha: marble.isDummy ? 0.6 : 1 });
    }
  }
